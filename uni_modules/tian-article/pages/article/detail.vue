	...
	<view>
		<view class="bg-white container">
			<view class="title">
				...
				<view class="sub-title ti-flex">
					<text class="margin-right-30" v-if="item.source_from">{{ item.source_from }}</text>
					<uni-dateformat class="font-20" :date="item.publish_date" format="yyyy-MM-dd"
						:threshold="[60000, 3600000]"></uni-dateformat>
					<!-- <text class="st-item">浏览：{{ item.view_count }}</text> -->
					<view class="flex-1">
					</view>
					<tian-icons type="biaoxingfill" @click="unFav" v-if="isFav" align="right" color="#FEA33B">已收藏
					</tian-icons>
					<tian-icons type="biaoxingfill" @click="fav" v-else align="right" color="#C6C6C6">收藏</tian-icons>
				</view>
			</view>
			<uni-load-more v-if="!loaded" status="loading" class="margin-top-60"></uni-load-more>
			<block v-if="loaded">
				<view class="video-content" v-if="item.videos && isOnShow">
					<view v-for="(video,index1) in item.videos" :key="index1" class="padding-top-20">
						<video :src="video" class="video" :autoplay="false"></video>
					</view>
				</view>
				<!-- #ifndef MP -->
				<ad v-if="topAdpid" :adpid="topAdpid" :unit-id="topAdpid" @load="adLoad" @error="adError"
					@close="adClose">
				</ad>
				<!-- #endif -->
				<view class="content">
			...
			<view class="text-center padding-40" v-if="canGetDoubleScore" @click="getDoubleScore(item.task, 'article')">
				<tian-icons type="chakanshipin" size="40" color="#eb4e38"><text
						class="font-36">{{item.task.name}}</text>
				</tian-icons>
			</view>
		</view>
		<!-- 同类型推荐 -->
		<view class="padding-22 bg-white margin-top-22" v-if="articleConfigs.category_id">
			<tian-article :configs="articleConfigs"> </tian-article>
		</view>
		...
		methods: {
			...
			loadData() {
				...
				db.action('article_read')
					...
					.field(
						'avatar,title,category_id,excerpt,publish_date,like_count,source_from,view_count,content,comment_count,comment_status'
					)
					.get({
						getOne: true
					})
			...
			checkFav() {
				db.collection('tian-mall-favorite')
					.where(`user_id==$cloudEnv_uid && type=='article' && id=="${this.item._id}"`)
					.field('_id')
					.get({
						getOne: true
					})
					...
					success: res => {
						if (res.confirm) {
							...
							db.collection("tian-mall-favorite").where(
									`user_id==$cloudEnv_uid && type=='article' && id=="${this.item._id}"`)
								.remove().then(() => {
									this.$success("取消成功")
									this.isFav = false;
								}).catch(() => {
			...
			fav() {
				if (this.isLogin) {
					...
					let video = this.item.videos ? this.item.videos[0] : undefined
					db.collection("tian-mall-favorite").add({
						id: this.item._id,
						title: this.item.title,
						avatar: this.item.avatar,
						description: this.item.excerpt,
						video: video,
