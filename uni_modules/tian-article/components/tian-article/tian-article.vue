<template>
	<view class="article-container" :style="bgStyle" :class="[options.bg_class_name,options.mode]" @click="bgClick">
		<view v-for="(item, index) in dataList" :key="index">
			<view class="item" v-if="item.isBanner" :class="item.className" :style="item.style"
				...
				<image :src="item.avatar" mode="aspectFill" class="image" :style="item.imageStyle"></image>
				<view class="more ti-flex">
			...
			<view class="item" v-else :class="item.className" :style="item.style" @click.stop="navToDetail(item)">
				<image :src="item.avatar" mode="aspectFill" class="image" :style="item.imageStyle"></image>
				<view class="content">
					<view class="title text-2">
						<text>{{ item.title }}</text>
		...
		methods: {
			...
			async loadData() {
				...
				db.action('article_read_check')
					...
					.then(({
						...
						if (this.options.extId) {
							let extIndex = result.data.findIndex(e => e._id == this.options.extId);
							if (extIndex > -1) {
								result.data.splice(extIndex, 1)
							}
						}
			...
			buildData(data) {
				...
				data.forEach((item, index) => {
					...
					item.isBanner = item.layout.className == "banner";
					...
					if (item.layout.image) {
						let [width, height] = item.layout.image.split("x")
						item.imageStyle = `width:${width}rpx;height:${height}rpx;`
					}
