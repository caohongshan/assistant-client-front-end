<template>
	<view class="search-container" :class="{fixed:configs.fixed,isSeat:configs.isSeat}">
		<view class="uni-searchbar" :style="pageStyle">
			...
			<view :style="searchStyle" class="uni-searchbar__box" @click="searchClick">
		...
		<view class="search-tmp" :style="pageTmpStyle">
...
<script>
	...
	export default {
		...
		computed: {
			searchStyle() {
				return `border-radius:${this.radius}px;background-color:${this.configs.searchBackground}`
			},
			userInfo() {
			...
			pageStyle() {
				...
				if (top && this.configs.fixed) {
					let style = [`padding-top:${top}px;`]
					console.log(this.tianPage)
					let bgImg = ""
					if (this.tianPage && this.tianPage.configs && this.tianPage.configs.configs && this.tianPage.configs
						.configs.backgroundImg) {
						style.push(
							`background-image:url(${this.tianPage.configs.configs.backgroundImg}) ;background-repeat:no-repeat;background-size: 100%;`
						)
					}

					//第一个元素
					if ((this.tianPage && this.tianPage.dataList && this.tianPage.dataList.length > 0 && this.tianPage
							.dataList[0].component == "tian-search-bar") || this.configs.isFirst) {
						let space = windowWidth - left - width;
						let paddingRight = width + space * 2
						style.push(
							`padding-right:${paddingRight}px;width:${windowWidth-paddingRight-space}px;padding-left:${space}px`
						)
					}
					return style.join(";")
				}
				// #endif
				return `background-color:${this.bgColorText}`
			},
			pageTmpStyle() {
				let {
					height,
				} = this.systemInfo.custom;
				if (height && this.configs.fixed) {
					//app和小程序height高度不一致
					//#ifndef MP
					height += 6
					//#endif
					return `height:${height+10}px;`
				}
			}
		},
		mounted() {
