<template>
	<view class="gride-container" :class="[options.display,options.bgClassName]">
		<uni-grid :column="options.column" :showBorder="false" :square="false">
			<uni-grid-item v-for="(gi,ind3) in items" :key="ind3">
				<view class="item" @click="itemNavTo(gi.open_url,ind3)" :class="options.itemClassName">
					...
					<text v-if="gi.text" class="">{{gi.text}}</text>
					<view class="" v-if="gi.description">
						<text class="font-20" :style="{color: options.descriptionColor}">{{gi.description}}</text>
					</view>
				</view>
			</uni-grid-item>
		</uni-grid>
