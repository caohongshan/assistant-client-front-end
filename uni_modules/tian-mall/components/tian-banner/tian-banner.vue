<template>
	<view class="banner-container" :class="[options.display]">
		<view class="" v-if="dataList.length==1" @click="navToItem(dataList[0],0)">
			<image :src="dataList[0].url" mode="aspectFill" class="image" :class="options.itemClassName"
				:style="{height:options.height+'rpx',width:options.width+'rpx'}">
			</image>
		</view>
		<uni-swiper-dot v-else :info="dataList" :current="current" field="content" :mode="options.mode">
			<swiper class="swiper-box" :class="options.style" autoplay :interval="options.interval"
	...
	export default {
		...
		methods: {
			...
			buildData(data) {
				...
				data.forEach(item => {
					//微信中不支持使用过滤器
					item.url = this.$thumbImg(item.src,
