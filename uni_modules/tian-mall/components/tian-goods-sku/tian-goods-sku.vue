<template>
	<view class="goods-sku-container">
		<uni-popup ref="skuPopup" :animation="!0" :maskClick="!0" type="bottom" @change="change" :safe-area="false">
			<view class="panel" :class="{single:single}">
				<view class="goods-info">
					<view class="goods-list" v-if="selectSku._id">
						...
						<view class="goods-content">
							<tian-goods-price :value="selectSku" :hasCart="false" />
					...
					<view class="goods-list" v-else>
						...
						<view class="goods-content">
							<tian-goods-price :value="item" :hasCart="false" />
							<view class="font-26 colorGray">
					...
					<tian-icons type="guanbi2" @click="close()" size="26" />
				</view>
				<uni-load-more v-if="!loaded" status="loading" />
				<scroll-view scroll-y="true" class="sku-content" v-if="loaded">
					...
					<view class="amount">
						<text>数量</text>
						<view class="">
							<uni-number-box :min="1" :max="99999" :step="1" v-model="amount" />
						</view>
					</view>
				</scroll-view>
				<view class="margin-top-22">
					<tian-goods-nav v-if="isSubmitSku" :options="[]" :buttonGroup="submitButtonGroup" :fill="true"
						@buttonClick="buttonClick({index:isSubmitSku-1})" />
					<tian-goods-nav v-else :options="[]" :fill="true" @buttonClick="buttonClick" />
				</view>
			</view>
		</uni-popup>
	</view>
		...
		methods: {
			loadData() {
				...
				this.$request('mall/cart/getsku', { 
					...
					this.stock = 0;
					let default_select_sku_id = this.sid
					result.map(item => {
						let sku_id = item._id;
						// delete item._id;
						let miaosha = {
						...
						//处理规格的秒杀
						if (miaosha && miaosha.sku_id == item._id) {
							...
							if (miaosha.is_begin) {
								...
								if (miaosha.price) {
									//秒杀一口价
									item.price = miaosha.price;
								} else {
									item.price = parseInt(item.price * miaosha.discount / 100)
								}
							}
							//如果不是强制选中规格，则默认此规格
							if (!default_select_sku_id) {
								default_select_sku_id = item._id
							}
						...
						//默认选中
						if (default_select_sku_id == item._id) {
						...
						if (names.length > 1) {
							...
							//修复从购物车点过来，规格不能切换的bug
							nameHash = nameHash.concat(names)
						}
					})
					let selectNameMap = {}
					//选中规格
